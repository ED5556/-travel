...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['action']) && $_POST['action'] == 'edit_reply') {
    ...
    $sql = "UPDATE replies SET reply_text = '$new_reply_text' WHERE id = $reply_id";
    if ($conn->query($sql) === TRUE) {
        echo "回覆修改成功<br>";
    } else {
        echo "錯誤: " . $conn->error;
            ...
            while ($row = $result_reviews->fetch_assoc()) {
                ...
                if ($result_replies->num_rows > 0) {
                    ...
                    while ($reply = $result_replies->fetch_assoc()) {
                        echo "<li>" . $reply['reply_text'] . " 
                            <form action='index.php' method='POST' style='display:inline'>
                                <input type='hidden' name='action' value='edit_reply'>
                                <input type='hidden' name='reply_id' value='" . $reply['id'] . "'>
                                <input type='text' name='new_reply_text' value='" . $reply['reply_text'] . "' required>
